...
> ⚠️ **本指南為手動部署參考，不受 Terraform 管理。**
> HedgeDoc 並未整合至此專案的 Terraform 設定（`main.tf` 中無對應 resource）。
> 本文件描述如何在既有的 OKE 叢集上**手動**部署 HedgeDoc，
> 使用 `k8s/hedgedoc-namespace.yaml` 和 `k8s/hedgedoc-secrets.yaml` 作為起點。

> **目標：** 在現有 OKE Always Free 叢集上，透過手動 Helm 指令部署 HedgeDoc（協作式 Markdown 編輯器），
